...
注意：
    ...
    - 建议首次测试先用 --strategy base_cot 验证连通性
"""
import argparse
import json
import os
...
def build_cmd(strategy_name, extra_args):
    """构建 harness.py 命令。"""
    return [
...
def run_single(strategy_name, extra_args, verbose=True):
    """运行单个策略实验，返回结果摘要。"""
    cmd = build_cmd(strategy_name, extra_args)
    if verbose:
        print(f"\n{'='*60}")
...
def run_all_serial(experiments):
    """串行运行所有实验。"""
    print(f"\n{'#'*60}")
    ...
    for name, args in experiments:
        ...
        if name == "step_verifier":
            print("⚠️ 警告: step_verifier 非常慢（约 5min/条），50 条约需 4~5 小时。")
...
def run_all_parallel(experiments, max_workers=3):
    """并行运行实验（可能触发API限流）。"""
    print(f"\n{'#'*60}")
...
def main():
    parser = argparse.ArgumentParser(description="Run COT experiments with 50 samples")
    ...
    if args.strategy:
        ...
        if not found:
            ...
            sys.exit(1)
        res = run_single(found[0][0], found[0][1])
        print_summary([res])
        return

    # 跑全部策略
    if args.parallel:
        results = run_all_parallel(EXPERIMENTS, max_workers=args.max_workers)
    else:
        results = run_all_serial(EXPERIMENTS)
