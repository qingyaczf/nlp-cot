...
\begin{itemize}
  ...
  \item \textbf{Prefix Consistency}（Iwase et al., 2026）~\cite{yu2025pc}：通过截断-再生机制评估推理链的可靠性。
  \item \textbf{Step-Aware Verifier}（Li et al., 2023）~\cite{li2023verifier}：引入步骤级验证器对推理链的每一步进行正确性评估，筛选高质量推理路径。
  \item \textbf{RAG + COT}（Trivedi et al., 2023）~\cite{trivedi2022ragcot}：结合外部知识库检索，为推理提供上下文支持。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{self-consistency.png}
\caption{Self-Consistency 策略示意图}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{prefix-consistency.png}
\caption{Prefix Consistency 策略示意图}
\label{fig:prefix-consistency}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{verifier.png}
\caption{Step-Aware Verifier 策略示意图}
\label{fig:verifier}
\end{figure}
...
RAG + COT（Retrieval-Augmented Chain-of-Thought）借鉴 IRCoT~\cite{trivedi2022ragcot} 的思想，在生成推理链前先从外部知识库检索与问题相关的数学知识，将其注入 prompt 中辅助推理。本节从 \textbf{Harness Engineering} 视角刻画该策略：检索器作为 \textbf{Tools} 子系统、prompt 作为 \textbf{Instructions} 子系统、检索上下文作为 \textbf{State} 子系统。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{rag.png}
\caption{RAG + COT 策略示意图}
\label{fig:rag}
\end{figure}

\subsubsection{两阶段流程与 Prompt 构造}

设输入问题为 $q$，选项集合 $O = \{o_1, \ldots, o_5\}$，外部知识库为 $\mathcal{K} = \{d_1, d_2, \ldots, d_{|\mathcal{K}|}\}$，每条 $d_i$ 包含一段 \texttt{content} 文本和一个 \texttt{source} 标签。RAG + COT 将推理过程分为两个阶段：

\textbf{阶段 1 -- 检索（Retrieve）}：调用检索器 $\mathrm{retrieve}(\cdot)$ 从 $\mathcal{K}$ 中召回与 $q$ 最相关的 $k$ 条文档，得到上下文集合
\begin{equation}
\mathcal{C}_q = \mathrm{retrieve}(q, k) = \{d_{(1)}, d_{(2)}, \ldots, d_{(k)}\},
\quad d_{(i)} \in \mathcal{K}.
\end{equation}

\textbf{阶段 2 -- 推理（Reason）}：将 $\mathcal{C}_q$ 拼接进 prompt 模板 $T_{\text{rag}}$，与原题 $q$、选项 $O$ 一起送入模型生成推理链 $y$，并由答案抽取函数 $f_{\text{extract}}(y)$ 得到最终预测：
\begin{equation}
P_{\text{RAG}} = T_{\text{rag}}\bigl(\,\mathrm{format}(\mathcal{C}_q),\, q,\, O\,\bigr),
\qquad
\hat{a} = f_{\text{extract}}\bigl(M.\mathrm{generate}(P_{\text{RAG}})\bigr).
\end{equation}
其中 $\mathrm{format}(\mathcal{C}_q)$ 将每条文档按 \texttt{[i] $d_{(i)}$.content} 的格式拼接成多行字符串。

\subsubsection{检索器实现：基于 Jaccard 相似度的 Keyword Retriever}

本项目采用轻量级关键词检索器 \texttt{SimpleKeywordRetriever}，避免引入向量数据库等重型依赖，便于复现实验。检索过程包含三步：

\textbf{(1) 分词与停用词过滤}：对查询 $q$ 和每条文档 $d \in \mathcal{K}$ 应用相同的分词函数 $\mathrm{tok}(\cdot)$，
\begin{equation}
\mathrm{tok}(s) = \{\, t \mid t \in \mathrm{regex}(s,\,\text{[a-z]+}),\ t \notin \mathcal{S},\ |t| > 2\,\},
\end{equation}
其中 $\mathcal{S}$ 为约 100 个英文停用词组成的固定集合（包含冠词、介词、代词、常用副词及 \texttt{one}\textendash\texttt{ten} 数词），$|t| > 2$ 用于过滤过短的无信息 token。

\textbf{(2) Jaccard 相似度评分}：以分词后的集合作为词袋表示，计算查询 $q$ 与文档 $d$ 的相似度
\begin{equation}
\mathrm{sim}_{\text{Jaccard}}(q, d)
= \frac{|\mathrm{tok}(q) \cap \mathrm{tok}(d)|}{|\mathrm{tok}(q) \cup \mathrm{tok}(d)|}
\in [0, 1].
\end{equation}
其中分母为查询和文档 token 的并集大小。当 $\mathrm{tok}(d) = \emptyset$ 或并集为空时，相似度记为 $0$。

\textbf{(3) Top-$k$ 排序与截断}：仅保留得分严格大于 $0$ 的候选文档，按相似度降序取前 $k$ 条返回：
\begin{equation}
\mathcal{C}_q = \mathop{\mathrm{Top}\text{-}k}_{d \in \mathcal{K}}\bigl\{\,d \mid \mathrm{sim}_{\text{Jaccard}}(q, d) > 0\,\bigr\}.
\end{equation}
当所有文档得分均为 $0$ 或知识库为空时，返回空集合，此时 prompt 退化为纯 base CoT 形式。

\subsubsection{Prompt 模板设计}

实际使用的 \texttt{rag\_cot.txt} 模板如下（节选）：
...
\caption{RAG + COT 策略}
\begin{algorithmic}[1]
\Require 问题 $q$, 选项 $O$, 模型 $M$, 检索器 $R$, 知识库 $\mathcal{K}$, 模板 $T_{\text{rag}}$, 检索数 $k=3$
\Ensure 预测答案 $\hat{a}$
\State $\mathcal{C}_q \gets R.\text{retrieve}(q, k)$ \Comment{阶段 1：基于 Jaccard 相似度的 Top-$k$ 检索}
\If{$\mathcal{C}_q = \emptyset$}
    \State $\text{context} \gets \text{``''}$ \Comment{无相关知识，退化为 base CoT}
\Else
    \State $\text{context} \gets$ \textsc{FormatDocs}$(\mathcal{C}_q)$ \Comment{按 \texttt{[i] content} 拼接}
\EndIf
\State $P \gets T_{\text{rag}}.\text{format}(q, O, \text{context})$ \Comment{阶段 2：构造增广 prompt}
\State $y \gets M.\text{generate}(P, \text{temperature}=0.7, \text{max\_tokens}=1024)$
\State $\hat{a} \gets \text{extract\_answer}(y)$
\State \Return $\hat{a}$
\end{algorithmic}
\end{algorithm}
...
实验中使用的知识库 \texttt{knowledge\_base.json} 含 \textbf{15} 条手工整理的通用数学事实，每条形如 \texttt{\{"content": "...", "source": "..."\}}，按 \texttt{source} 归类为：\texttt{math\_basics}（折扣、百分比、利息、利润、速度--时间、工作率、比例、混合等约 8 条）、\texttt{geometry}（面积/周长公式）、\texttt{probability}（概率与独立事件公式）等。该知识库设计为\textbf{可热插拔}：检索器以 \texttt{knowledge\_path} 参数加载 JSON，运行时若文件缺失则自动初始化为空列表，不会中断流程。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{multi-agent-debate.png}
\caption{Multi-Agent Debate 策略示意图}
\label{fig:multi-agent-debate}
\end{figure}
