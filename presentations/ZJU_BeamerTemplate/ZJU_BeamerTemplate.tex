  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.52\textwidth}
      \begin{block}{模型架构与评分}
        ...
        \end{itemize}
        \vspace{0.2em}
        \texttt{score = softmax(logits[0,0])[SOLUTION-CORRECT] $\times$ 10}
      \end{block}
      \begin{block}{多 Prompt + 加权投票}
        \small
        \texttt{--n\_prompts}：每道题用多个不同 random few-shot prompt，每个采 \texttt{--n\_paths} 条路径。
        \[
    ...
    \begin{column}{0.44\textwidth}
      ...
      \end{alertblock}
      \begin{block}{模型分发（HuggingFace）}
        \footnotesize
        \centering
        \texttt{huggingface.co/k1r1same/aqua-verifier}\\
        （国内自动镜像 hf-mirror）
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{方法对比总结：不同策略解决不同失败模式}
  ...
  \begin{tabular}{p{2.5cm} p{3.5cm} p{4.5cm} p{4.5cm}}
    ...
    \addlinespace[2pt]
    Step Verifier (LLM) & 中间步骤错误 & 可解释性强，能定位 step 级问题 & 耗时极高 \\
    \addlinespace[2pt]
    Step Verifier (DeBERTa) & 路径质量评分 & 验证阶段本地推理，零 API & 训练数据风格不匹配 \\
    \addlinespace[2pt]
